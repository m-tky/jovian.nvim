...
| `ui/math.lua` | Self-contained LaTeX→Unicode converter for `$…$`/`$$…$$` math (Greek, operators, super/subscripts, `\frac`/`\sqrt`/`\mathbb`, …); optional external `math.converter` (latex2text/utftex) hook |
