...
local function row_chunk(row, cols, hl)
...
function M.ensure_transmitted(b64, cb, cols, rows)
    ...
    Core.on_kitty_ready(function(ok, _attach_err)
        if not ok then
            _transmits[key] = nil
        ...
        client:request("kitty_transmit", { png_b64 = b64, cols = cols, rows = rows }, function(err, result)
