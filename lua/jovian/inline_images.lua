...
local function decode_and_save(b64, img_path)
    ...
    if vim.base64 and vim.base64.decode then
        ...
        if ok and decoded then
            ...
            if f then
                f:write(decoded)
                f:close()
                if vim.fn.getfsize(img_path) > 0 then
                    return true
                end
            end
        end
    ...
    local write_script =
        string.format("import base64, sys; open('%s', 'wb').write(base64.b64decode(sys.stdin.read()))", img_path)
...
function M.render_for_buffer(bufnr)
    ...
    local function scan_buffer_for_images(attachments_dict)
        ...
        for i, line in ipairs(lines) do
            ...
            if attachments_dict then
                ...
                if not attach_name then
                    attach_name = line:match("src=[\"']attachment:([^\"']+)[\"']")
                end
                ...
                if attach_name then
                    ...
                    if attachments_dict[safe_name] then
                        ...
                        if win_id then
                            ...
                            local img = image_api.from_file(
                                img_path,
                                { window = win_id, buffer = bufnr, y = target_y, with_virtual_padding = true }
                            )
                            if img then
            ...
            if not rendered then
                ...
                if in_data_uri then
                    ...
                    if end_idx then
                        ...
                        if win_id then
                            local target_y =
                                math.min(data_uri_line - line_offset + 1, vim.api.nvim_buf_line_count(bufnr) - 1)
                            local img = image_api.from_file(
                                img_path,
                                { window = win_id, buffer = bufnr, y = target_y, with_virtual_padding = true }
                            )
                    ...
                    if start_b64 then
                        ...
                        if full_match then
                            ...
                            if win_id then
                                local target_y =
                                    math.min(i - 1 - line_offset + 1, vim.api.nvim_buf_line_count(bufnr) - 1)
                                local img = image_api.from_file(
                                    img_path,
                                    { window = win_id, buffer = bufnr, y = target_y, with_virtual_padding = true }
                                )
    ...
    if filepath:match("%.ipynb$") then
        vim.loop.fs_stat(filepath, function(err, stat)
            ...
            vim.schedule(function()
                if stat.mtime.sec <= cache.mtime and #cache.images > 0 then
                    return
                end
